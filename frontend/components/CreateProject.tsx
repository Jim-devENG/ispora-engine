...
const projectTypes = [
  ...
  },
  {
    ...
    description: "General purpose team-based projects that don't fall neatly into the other categories — like tech builds, media, AI tools, etc.",
...
export function CreateProject({ onBack, onSave }: CreateProjectProps) {
  ...
  const [newPositionCategory, setNewPositionCategory] = useState<DiasporaPosition['category']>('support');
  ...
  const handleSave = async (isDraft: boolean = false) => {
    ...
    const projectData = {
        ...
        category: formData.category || formData.projectType,
        priority: formData.priority,
        status: isDraft ? 'planning' : 'active',
        startDate: formData.startDate,
        deadline: formData.endDate,
        university: formData.university,
        location: profile?.location || '',
        tags: formData.tags,
        mentorshipConnection: formData.mentorshipConnection,
        isPublic: formData.isPublic,
        teamMembers: teamMembers,
        diasporaPositions: diasporaPositions,
        aspiraCategory: formData.projectType === 'mentorship' ? 'mentorships' : 
                       formData.projectType === 'academic' ? 'research' :
                       formData.projectType === 'community' ? 'community-service' : 'university-campaigns',
      ...
      try {
        ...
        if (supabaseError?.message?.includes('Not authenticated') || supabaseError?.message?.includes('Auth session missing')) {
          ...
          // Don't reload - let the user see the error and handle it themselves
    ...
    if (onSave) {
      ...
      console.log("Project saved:", createdProject);
    } catch (error: any) {
  ...
  const canProceed = () => {
    switch (currentStep) {
      case 0: return selectedProjectType !== "";
      case 1: return formData.title.trim() !== "" && formData.description.trim() !== "";
      case 2: return true; // Team and positions are optional
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-muted-foreground">Choose the category that best describes your project to get relevant features and settings.</p>
            </div>
                ...
                return (
                  <Card 
                    ...
                    className={`cursor-pointer transition-all duration-200 hover:shadow-md ${
                      isSelected 
                        ? 'ring-2 ring-[#021ff6] bg-[#021ff6]/5' 
                        : 'hover:shadow-lg'
                    }`}
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            {isSelected && (
                              <CheckCircle className="h-5 w-5 text-[#021ff6]" />
                            )}
                          </div>
          ...
          <div className="space-y-6">
                      ...
                      <CommandList>
                        <CommandEmpty>
                          <div className="p-2 text-center">
                            <p className="text-sm text-muted-foreground mb-2">No institutions found.</p>
                            <Button
                              size="sm"
                              onClick={() => {
                        ...
                        {!isAddingCustomUniversity && universitySearchValue && filteredUniversities.length > 0 && (
                          <CommandGroup>
                            <CommandItem
                              onSelect={() => {
                                setIsAddingCustomUniversity(true);
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Define the key objectives that drive your program's mission and impact</p>
              </div>
              <div className="flex gap-2">
                <Input
                  id="objectives"
            ...
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">
                  Specify leadership and core team positions open to diaspora community members. These are for key roles, not general participants.
                </p>
              </CardHeader>
                ...
                <div className="space-y-4">
                      ...
                      <PopoverContent className="w-full p-0" align="start">
                        <Command>
                          ...
                          <CommandList>
                            <CommandEmpty>
                              <div className="p-2 text-center">
                                <p className="text-sm text-muted-foreground mb-2">No preset positions found.</p>
                                <Button
                                  size="sm"
                            ...
                            <CommandGroup>
                              <CommandItem
                                onSelect={() => setIsAddingCustomPosition(true)}
                              >
                                <Plus className="mr-2 h-4 w-4" />
                                Create Custom Position
                              </CommandItem>
                          ...
                          <div className="p-3 border-t border-border">
                            <div className="space-y-3">
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                <div className="space-y-2">
                                  <Label htmlFor="custom-position-commitment">Time Commitment</Label>
                                  <Input
                                ...
                                <div className="space-y-2">
                                  ...
                                  <Select value={newPositionCategory} onValueChange={(value: DiasporaPosition['category']) => setNewPositionCategory(value)}>
                                    <SelectTrigger>
                                      <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                  ...
                  {diasporaPositions.length > 0 && (
                    <div className="space-y-3">
                      <div className="flex items-center gap-2 mb-3">
                        <Badge variant="outline" className="bg-[#021ff6]/10 text-[#021ff6] border-[#021ff6]/20">
                          {diasporaPositions.length} Position{diasporaPositions.length !== 1 ? 's' : ''} Available
                        </Badge>
                      ...
                      {diasporaPositions.map((position) => {
                        const Icon = getCategoryIcon(position.category);
                        return (
                          <div key={position.id} className="border rounded-lg p-4 bg-gradient-to-r from-blue-50/50 to-purple-50/50">
                            <div className="flex items-start justify-between">
                              <div className="flex items-start gap-3 flex-1">
                                <div className={`p-2 rounded-lg ${getCategoryColor(position.category)}`}>
                                  ...
                                  <div className="flex items-center gap-4 text-xs text-muted-foreground">
                                    ...
                                    {position.responsibilities.length > 0 && (
                                      <span className="flex items-center gap-1">
                                        ...
                                        {position.responsibilities.length} responsibilit{position.responsibilities.length !== 1 ? 'ies' : 'y'}
                                      </span>
                                    )}
                                  </div>
                                </div>
                  ...
                  {diasporaPositions.length === 0 && (
                    <div className="text-center py-8 border-2 border-dashed border-gray-300 rounded-lg">
                      ...
                      <h4 className="font-medium text-gray-900 mb-2">No Diaspora Positions Added Yet</h4>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Enable mentorship features for this project</p>
                      {selectedType?.defaultConnections.mentorshipConnection && (
                        <p className="text-xs text-blue-600">Recommended for {selectedType.label} projects</p>
                      )}
                    </div>
                    <Switch
                      id="mentorship-connection"
                      checked={formData.mentorshipConnection}
                      onCheckedChange={(checked) => handleInputChange("mentorshipConnection", checked)}
                    />
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Allow others to discover and request to join this project</p>
                  </div>
                  <Switch
                    id="public-project"
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {currentStep === steps.length - 1 && (
              <Button onClick={() => handleSave(false)} disabled={isSaving} className="bg-[#021ff6] hover:bg-[#021ff6]/90">
        ...
        <div className="flex items-center justify-between">
          {steps.map((step, index) => (
            <div key={index} className="flex items-center">
              <div className={`flex items-center gap-2 ${index <= currentStep ? 'text-[#021ff6]' : 'text-gray-400'}`}>
              ...
              {index < steps.length - 1 && (
                <div className={`w-8 h-px mx-4 ${index < currentStep ? 'bg-[#021ff6]' : 'bg-gray-200'}`} />
              )}
            </div>
          ))}
        </div>
            ...
            <div className="flex items-center gap-2">
              {currentStep < steps.length - 1 ? (
                ...
                <Button
                  ...
                  disabled={isSaving}
                  className="bg-[#021ff6] hover:bg-[#021ff6]/90 min-w-[140px]"
                >
                  <Send className="h-4 w-4 mr-2" />
                  {isSaving ? 'Creating...' : 'Create Project'}
                </Button>
