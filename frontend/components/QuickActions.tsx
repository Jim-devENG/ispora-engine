...
function ActionCard({ title, description, icon, iconColor, onAction, badgeCount }: ActionCardProps) {
  return (
    <Card className="hover:shadow-md transition-shadow cursor-pointer relative" onClick={onAction}>
      ...
      <CardHeader className="space-y-4">
        <div className="flex items-center justify-between">
          <div className={`h-8 w-8 rounded-full bg-opacity-10 flex items-center justify-center ${iconColor}`}>
            {icon}
          </div>
...
export function QuickActions() {
  ...
  return (
    <>
      ...
      <AlmaMaterConnection 
        isOpen={isAlmaMaterOpen}
        onClose={() => setIsAlmaMaterOpen(false)}
      />
