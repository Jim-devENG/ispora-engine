...
const mockRecordings: Recording[] = [
  {
    ...
    description:
      'A comprehensive walkthrough of feature engineering techniques for machine learning projects',
    duration: 1245, // 20:45
    url: '#',
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <div className="flex items-start justify-between">
              ...
              <Badge className={statusColors[recording.status]}>{recording.status}</Badge>
            </div>
...
export function VideoRecorder({ mentee }: VideoRecorderProps) {
  ...
  const handleSaveRecording = (title: string, description: string, tags: string[]) => {
    ...
    setTimeout(() => {
      setRecordings((prev) =>
        prev.map((r) => (r.id === newRecording.id ? { ...r, status: 'ready' as const } : r)),
      );
    }, 3000);
  };
