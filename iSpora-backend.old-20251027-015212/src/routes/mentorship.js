...
router.get('/', protect, async (req, res, next) => {
  try {
    ...
    let query = db('mentorships as m')
      ...
      .where(function () {
        this.where('m.mentor_id', userId).orWhere('m.mentee_id', userId);
      });
...
router.put('/:id', protect, async (req, res, next) => {
  try {
    ...
    const mentorship = await db('mentorships').where({ id }).first();
    ...
    await db('mentorships').where({ id }).update(updateData);
