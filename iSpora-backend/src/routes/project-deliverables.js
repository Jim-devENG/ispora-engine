  ...
  try {
    ...
    const deliverable = await db('project_deliverables as pd')
      ...
      .leftJoin('project_members as pm', function() {
        this.on('pd.project_id', '=', 'pm.project_id')
    ...
    const canUpdate = deliverable.created_by === req.user.id || 
...
router.post('/:deliverableId/submit', protect, async (req, res, next) => {
  try {
    ...
    if (deliverable.assigned_to !== req.user.id && !['admin', 'mentor'].includes(deliverable.role)) {
      return res.status(403).json({
        success: false,
...
router.post('/:deliverableId/review', protect, async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: requires_revision ? 'Deliverable marked for revision' : 'Deliverable reviewed successfully'
    });
  } catch (error) {
    next(error);
  }
...
router.get('/:projectId/analytics', protect, async (req, res, next) => {
  try {
    ...
    const [
      ...
      db('project_deliverables').where({ project_id: projectId, is_template: false }).count('* as count').first(),
      ...
      db('project_deliverables')
        ...
        .select([
          ...
          db.raw('SUM(CASE WHEN status = \'submitted\' OR status = \'reviewed\' THEN 1 ELSE 0 END) as submitted_count'),
    ...
    const submissionRate = submissionStats.total > 0 ? 
      (submissionStats.submitted_count / submissionStats.total * 100).toFixed(1) : 0;
