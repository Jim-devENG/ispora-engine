...
async function createMissingTables() {
  try {
    ...
    await db.schema.createTable('project_sessions', function(table) {
      ...
      table.string('type', 50).defaultTo('meeting'); // meeting, workshop, presentation
    ...
    await db.schema.createTable('impact_feed', function(table) {
      ...
      table.text('description').nullable();
      table.string('impact_category', 50).notNullable(); // education, health, economic, social, environment
      table.string('location', 100).nullable();
      table.integer('people_impacted').nullable();
      table.decimal('monetary_impact', 15, 2).nullable();
      table.json('metrics').nullable(); // Additional impact metrics
    ...
    await db.schema.createTable('session_attendees', function(table) {
      ...
      table.uuid('user_id').notNullable().references('id').inTable('users').onDelete('CASCADE');
      table.string('status', 20).defaultTo('invited'); // invited, accepted, declined, attended
